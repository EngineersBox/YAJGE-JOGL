...
uniform float zFar;
uniform int shadowsOnly;
uniform int depthOnly;
uniform int showCascades;
uniform int debug;
#define DEBUG_COLOUR vec4(1, 0, 0, 1)
...
void main() {
    if (depthOnly == 1) {
        float z = 1.0 - (gl_FragCoord.z / gl_FragCoord.w) / zFar;
        fragColor = vec4(z, z, z, 1.0);
        return;
    }
    if (debug == 1) {
        fragColor = DEBUG_COLOUR;
        return;
    }
    ...
    float shadow = calcShadow(mlightviewVertexPos[cascadeIndex], cascadeIndex);
    if (shadowsOnly == 1) {
